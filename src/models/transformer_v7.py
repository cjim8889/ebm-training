...
from src.utils.models import init_linear_weights, xavier_init, zero_init
...
class TimeEmbedding(eqx.Module):
    net: eqx.nn.MLP
    d_net: Optional[eqx.nn.Linear] = None
    ...
    def __init__(self, 
        ...
        net_key, d_key = jax.random.split(key, 2)
        self.net = eqx.nn.MLP(
            in_size=frequency_embedding_size + embedding_size if shortcut else frequency_embedding_size,
    ...
    def __call__(self, t: Float[Array, ""], d: Optional[Float[Array, ""]] = None,) -> Float[Array, "hidden_size"]:
        ...
        if d is not None:
            ...
            t_freq = jnp.concatenate([t_freq, d_emb], axis=-1)
...
class EmbedderBlock(eqx.Module):
    ...
    def __call__(
        ...
        embedded = jax.vmap(self.layernorm)(self.mp_policy.cast_to_param(embedded))
        embedded = self.mp_policy.cast_to_compute(embedded)
        return embedded
...
###############################################################################
class ParticleTransformerV7(eqx.Module):
    """
    ...
    def __init__(
        ...
        self.predictor = FinalLayer(embedding_size, n_spatial_dim, key=p_key, mp_policy=mp_policy)
